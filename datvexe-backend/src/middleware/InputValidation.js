...
const inputValidationAccount = async (req, res, next) => {
  try {
    ...
    if (!username || !email || !phone) {
      return res
        .status(400)
        .json({ status: 400, message: "Thiếu thông tin đăng ký" });
    }
...
const inputValidationLogin = async (req, res, next) => {
  ...
  try {
    ...
    if (!username || !password) {
      return res
        .status(400)
        .json({ status: 400, message: "Thiếu tài khoản hoặc mật khẩu" });
    }
    ...
    if (!user) {
      return res
        .status(401)
        .json({ status: 401, message: "Tài khoản không tồn tại" });
    }
    if (!user) {
      throw new Error(
    ...
    if (!isMatch) {
      return res
        .status(401)
        .json({ status: 401, message: "Mật khẩu không chính xác" });
    }
