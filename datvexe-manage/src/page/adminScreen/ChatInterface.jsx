...
const ChatInterface = () => {
    ...
    if (selectedUser && socketRef.current) {
      ...
      pollingIntervalRef.current = setInterval(() => {
        if (socketRef.current && socketRef.current.connected) {
          ...
          socketRef.current.emit("get_conversation", {
  ...
  useEffect(() => {
    ...
    socketRef.current.on("receive_message", (data) => {
      try {
        ...
        if (
          data.sender.role === "USER" &&
          (!selectedUser || data.sender.id !== selectedUser.id)
        ) {
          setUsers((prevUsers) =>
            prevUsers.map((user) =>
  ...
  const handleSelectUser = async (user) => {
    ...
    try {
      ...
      notifyError(
        "Không thể tải tin nhắn: " + (error.message || "Lỗi không xác định")
      );
      setIsLoading(false);
    }
  };
                ...
                >
                  <ListItemAvatar>
                    ...
                    >
                      ...
                      >
                        {userNames[user.id] && userNames[user.id][0]
                          ? userNames[user.id][0].toUpperCase()
                          : "U"}
                      </Avatar>
                    </StyledBadge>
                  </ListItemAvatar>
            ...
            <>
              <ChatHeader>
                <Avatar sx={{ bgcolor: (theme) => theme.palette.primary.main }}>
                  {userNames[selectedUser.id] && userNames[selectedUser.id][0]
                    ? userNames[selectedUser.id][0].toUpperCase()
                    : "U"}
                </Avatar>
                <Box>
