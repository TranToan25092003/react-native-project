...
const popularDestinations = [
  {
    ...
    start: "MY_DINH_STATION_HN",
    end: "LAO_CAI_STATION_LC",
  },
  {
    id: 2,
    title: "Lào Cai - Sapa",
...
const PopularDestinations = () => {
  const getSchedule = async (start, end) => {
    const benXeKhoiHanh = start;
    const benXeDichDen = end;
  ...
  const renderDestination = async ({ item }) => {
    return (
      <TouchableOpacity
        style={styles.destinationCard}
        onPress={async () => {
          const resData = await getSchedule(item.start, item.end);
          // console.log(resData);
          navigation.navigate("CarScheduleScreen", {
            dataSchedule: resData,
          });
        }}
      >
