      ...
      const fetchData = async () => {
        try {
          ...
          const promotionResponse = await fetch(
            `http://10.0.2.2:9999/promotion`,
            {
              method: "GET",
            }
          );

          setPromotions((await promotionResponse.json()).data);
      ...
      {activeTab == "events" ? (
        ...
        <ScrollView style={styles.notificationsList}>
          {promotions.map((promotion) => (
            <View key={promotion._id} style={styles.notificationItem}>
              <View style={styles.iconContainer}>
                <MaterialIcons
                  name={getIcon("success")}
                  size={24}
                  color={getIconColor(
                    `${promotion.status == "01" ? "success" : "cancel"}`
                  )}
                />
              </View>
              <View style={styles.contentContainer}>
                <Text style={styles.title}>CODE: {promotion.code}</Text>
                <Text style={styles.message}>{promotion.description}</Text>
